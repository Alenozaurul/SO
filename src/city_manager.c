...
char *getDistrict(int argc, char *argv[]) {
	char commands[8][32] = {"--list", "--remove_report", "--add", "--filter", "--view",
...
int openDistrict(char *path) {
	if(mkdir(path, 0750) == 0) {
		...
		strcat(link_path, "report.dat");

		char link_name[64];
		strcpy(link_name, "active_reports-");
		strcat(link_name, district);

		if(symlink(link_path, link_name) < 0) {
			perror("Cannot create link to district\n");
	       		return 1;
		}		
	}
...
pid_t getPID() {
	int fd; 
	pid_t pid;
	char *path = "../.monitor_pid";
	
	if(stat(path, &st) == 0) {
		if( (fd = open(path, O_RDONLY)) == -1 ) {
			perror("Cannot open monitor_pid\n");
			return -1;
		}
		
		if(read(fd, &pid, sizeof(pid)) != sizeof(pid_t)) {
			printf("Error reading pid from file\n");
			close(fd);
			return -1;
		}
	} else {
		printf("Can't read monitor_pid\n");
		return -1;
	}
...
void addReport(){
	...
	if( (pid == -1) || (kill(pid, SIGUSR1) < 0) ) {
		char msg[256];
		sprintf(msg, "The monitor hasn't been notified about the report\n");
		write(log_file, &msg, strlen(msg));
	} else {
		char msg[256];
		sprintf(msg, "The monitor has been notified about the report\n");
		write(log_file, &msg, strlen(msg));
...
void removeDistrict() {
	...
	if(!pid) {
		...
		strcpy(remove, "rm -f active_reports-");
		strcat(remove, district);
